...
def downloadOneImg(url, path, proxies, headers):
	...
	fp = open(path + '..\\assets\\' + imageName, 'wb')
	fp.write(img)
	fp.close()
...
def getLastTime(path):
    file = open(path + '..\\python\\上一次导运行这个代码的时间', encoding='utf-8')
    lastTime = file.read()
    file.close()
...
def saveTheTimeOfToAnki(path, time):
    file = open(path + '..\\python\\上一次导运行这个代码的时间', 'w', encoding='utf-8')
    file.write(str(time))
    file.close()
			...
			if markdown_link_list:
				for markdown_link in markdown_link_list:
					...
					if re.search(r'(http.*?)\)', markdown_link):
						...
						content = content.replace(markdown_link, '![[' + imageName + ']]')
